...
def create_quot_pdf(quotation_id):
	...
	h=open("/home/frappe/frappe-bench/apps/strawberi/strawberi/quotation2.html", 'r')
	quotation2=str(h.read())
	i=open("/home/frappe/frappe-bench/apps/strawberi/strawberi/header_data.html", 'r')
	header=str(i.read())
	header=header.replace("quotation_id",quotation_id)
	...
	for q in quot_details:
		...
		quotation=quotation.replace("discount_amount",str(q.discount_amount))
		quotation=quotation.replace("additional_discount_percentage",str(q.additional_discount_percentage))
		...
		if q.parent_product_bundle:
			...
			pb_details=frappe.db.sql("""select * from `tabProduct Bundle` pb join `tabProduct Bundle Attribute` pba on pb.new_item_code=pba.product_bundle where pb.new_item_code=%s""",(q.item_code),as_dict=1)
		...
		if sales_person:
			header=header.replace("agent_name",str(sales_person[0]["sales_person"]))
		else:
		...
		if pb_details:
			...
			places=frappe.db.sql("""select * from `tabPlace` where parent in (select item_code from `tabProduct Bundle Item` where parent=%s and category="Package") order by sequence_number""",(pb_details[0]["new_item_code"]),as_dict=1)
			place_name="";
			for p in places:
		...
		for i in itinerary:
			...
			if i.start_date:
				itinerary_details=itinerary_details.replace("date",str(i.start_date.strftime(" %d %b %Y ")))
			else:
				itinerary_details=itinerary_details.replace("date","")
	...
	with open("/home/frappe/frappe-bench/apps/strawberi/strawberi/header.html", "w") as file:
    		file.write(header)	
	...
	"""pdfkit.from_string(quotation+quotation2, "/home/frappe/frappe-bench/sites/site1.local/public/files/"+quotation_id+".pdf",{
   '--header-html':'/home/frappe/frappe-bench/apps/strawberi/strawberi/header.html'},{
   '--footer-html': '/home/frappe/frappe-bench/apps/strawberi/strawberi/footer.html'})"""
	pdfkit.from_string(quotation+quotation2, "/home/frappe/frappe-bench/sites/site1.local/public/files/"+quotation_id+".pdf",{'--header-html': '/home/frappe/frappe-bench/apps/strawberi/strawberi/header.html',
   '--footer-html': '/home/frappe/frappe-bench/apps/strawberi/strawberi/footer.html'})
	#print (quotation)
...
@frappe.whitelist()  
def get_margin():
	sales_margin=frappe.db.sql("""select * from tabSingles where doctype='Sales Margin'""",as_dict=1)
	max_sales_margin=0
	min_sales_margin=0
	for sm in sales_margin:
		if sm.field == "max_sales_margin":
			max_sales_margin=sm.value
		if sm.field == "min_sales_margin":
			min_sales_margin=sm.value
		
			
	margin_value={"min_sales_margin":min_sales_margin,"max_sales_margin":max_sales_margin}	
	
	return margin_value
@frappe.whitelist()  
def set_margin(min_sales_margin,max_sales_margin):
	
	frappe.client.set_value("Sales Margin", "Sales Margin", "min_sales_margin",min_sales_margin)
	frappe.client.set_value("Sales Margin", "Sales Margin", "max_sales_margin",max_sales_margin)
	
	return "Sales margin has been updated successfully"	
@frappe.whitelist()
def create_rfq_along_with_bundle():
	try:
		...
		if RFQ_ID:
			...
			for sup in suppliers:
				...
				row = doc.append('suppliers',{})
				row.supplier = sup["supplier"]
	...
	"product":product["Product_ID"],"qty":product["qty"],"title":product["Title"], "description":product["Description"]}
			...
			c_id=""
			cust_id=frappe.db.sql("""select name from `tabCustomer` where name=%s""",(customer_name),as_dict=1)
			if cust_id:
				c_id=cust_id[0]["name"]
			...
			parent_product_bundle_item = parent_product_bundle + "-%"
			...
			if last_child_item:
				...
				try:
					...
					pb_item_code=parent_product_bundle+"-C-"+str('%03d' % int_item_last_value )
				except ValueError as ve:
					pb_item_code=parent_product_bundle+"-C-001"
			else:
				pb_item_code=parent_product_bundle+"-C-001"
			...
			if item_code:
				...
				product_bundle_name=parent_product_bundle+"-C-"+str('%03d' % pd_last_value )
			else:
				product_bundle_name=parent_product_bundle+"-C-001"
...
def get_quotation_details():
	...
	try:
		...
		for item in doc.items:
			...
			item_details = {"item_code":item_code,"item_name":item.item_name,"qty":item.qty,"rate":item.rate,
						"amount":item.amount,"description":item.description}
			items_list.append(item_details)
			is_package = check_is_package(item_code)
			if (is_package):
				itin_list = get_itin_details(item_code)
				package_details = {"bundle_id":item_code, "parent_product_bundle": get_parent_bundle_id(item_code), "Items":itin_list}
		...
		if doc_type == "Quotation":		
			breakup=[]
			for quot in doc.quotation_details:
				breakup_details={"sharing_mode":quot.sharing_mode,"quantity":quot.quantity, "rate":quot.rate,"amount":quot.amount}
				breakup.append(breakup_details)
			breakup_value={"total":doc.quot_total,"breakup":breakup}
			quotation_details = {"for":quotation_type,"rfq_id":doc.request_for_quotation,
							...
							"grand_total":doc.grand_total,"offers":offer_list,"price_breakup":breakup_value,"notes":doc.notes,"creation_date":doc.creation},"package_details":package_details}
			...
			quotation_details = {"for":quotation_type,"rfq_id":doc.request_for_quotation,
							...
							"number_of_adult":doc.number_of_adult,"number_of_infant":doc.number_of_infant,"supplier":doc.supplier,"inquiry_id":doc.opportunity,
							"items":items_list,"total":doc.total,"taxes":tax_list,"tax_total":doc.total_taxes_and_charges,
							"grand_total":doc.grand_total,"offers":offer_list,"notes":doc.notes,"creation_date":doc.creation},"package_details":package_details}
	...
	diff = datetime.now() - startdatetime 
	logger.debug("getQuotationDetails: Time Taken : " + str(diff))
...
def get_rfq_details():
	...
	try:
		...
		rfq_details = {"RFQ_ID":doc.name,"price_category":doc.price_category,"sales_person":doc.sales_person,"inquiry_id":doc.opportunity,"customer_name":doc.customer_name,
							...
							"suppliers":suppliers,"notes":doc.notes,"creation_date":doc.creation,"bundle_id":product_bundle_id, "parent_product_bundle":parent_product_bundle,"items":items_list}
...
def get_package_price():
	...
	try:
		...
		product_details, est_price = get_estimated_price(bundle_id, traveling_on, currency, product_list)
		...
		package_price = {"bundle_id":bundle_id, "currency":currency, "estimated_price":est_price, "price_breakup":product_details}
	except travel_user_exception as e:
		logger.exception(e.error_category + ": " + e.error_code + ": " + e.error_msg + " DESCRIPTION :" + e.error_desc)
		err_info = 'Error: {}. {}, line: {}'.format(sys.exc_info()[0],sys.exc_info()[1],sys.exc_info()[2].tb_lineno)
		error_json = {"Status":"Error", "Error_code":e.error_code, "Error Message":e.error_msg, "Error Type":e.error_category, "Error Info":err_info}
	...
	diff = datetime.now() - startdatetime 
	logger.debug("get_package_price: Time Taken : " + str(diff))
...
def get_itin_details(item_code):
	itin_list = []
	all_catagories = ["Hotel", "Sightseeing", "Transfer"]
	product_bundle_id = item_code
	itin_days = tapiutils.execute_frappe_select_sql("""select distinct(day) from `tabItinerary  Details` where parent=%s and parenttype=%s order by day""",
							(product_bundle_id, "Product Bundle"), "quotation.py", "get_itin_details")
	...
	for product in product_list:
		...
		if (product_id):
			...
			if (product_price > 0):
				...
				product_detail = {"sharing_mode":product_name,"quantity":qty,"rate":product_price,"amount":total_product_price}
				product_details.append(product_detail)
			est_price = est_price + total_product_price
	return product_details, est_price
...
def create_quotation(doctype, reqData):
	...
	for input_item in input_items:
		...
		if ((doctype == "Quotation") and (not (rfq_id))):
			is_package = check_is_package(bundle_id)
			logger.debug("IS PACKAGE" + str(is_package))
			if (is_package):
				product_list = get_product_list(bundle_id)
				logger.debug("PRODUCT LIST")
				logger.debug(product_list)
				parent_bundle_id = get_parent_bundle_id(bundle_id)
				currency = get_bundle_currency(parent_bundle_id)
				logger.debug("Parent Bundle Id: " + parent_bundle_id + " Currency: " + currency)
				quotation_details, est_price = get_estimated_price(parent_bundle_id, travelling_on, currency, product_list)
	...
	logger.debug("quotation_details ")
	logger.debug(quotation_details)
	if (doctype == "Quotation" and len(quotation_details) > 0):
		req_json={
			"company": "Strawberi",
...
def update_quotation(doctype, reqData):
	...
	if (quotation_type == "supplier"):
		...
		frappe.db.sql("""delete from `tabQuotation Details` where parenttype = "Quotation" and parent= %s""", (quotation_id), as_dict=1)
		...
		if ((doctype == "Quotation") and (not (rfq_id))):
			...
			if (is_package):
				...
				for quotation_detail in quotation_details:
					doc.append("quotation_details", quotation_detail)
...
def get_product_list(bundle_id):
	product_list = []
	first_hotel=tapiutils.execute_frappe_select_sql("""select * from `tabItinerary  Details` where parent=%s and parenttype=%s and category='Hotel' and (product is NOT NULL and  product != '') order by day ASC LIMIT 1""",
								(bundle_id, "Product Bundle"), "quotation.py", "product_list")
	for hotel in first_hotel:
		day = hotel.day
		hotel_id = hotel.product
		if ((hotel_id) and (day)):
			product_name = get_product_name(hotel.product)
			product_details = {"product_id":hotel.product,"product_name":product_name,"qty":hotel.qty}
			product_list.append(product_details)
			...
			for hotel_variant in hotel_variants:
				if not (hotel_variant.product == hotel_id):
					product_name = get_product_name(hotel_variant.product)
					product_details = {"product_id":hotel_variant.product,"product_name":product_name,"qty":hotel_variant.qty}
					product_list.append(product_details)
...
def get_variant_product_list(bundle_id, day, hotel_list):
	...
	for hotel in hotel_list:
		...
		if (i == 1):
			...
			product_name = get_product_name(first_hotel_id)
			product_details = {"product_id":first_hotel_id,"product_name":product_name,"qty":hotel["qty"]}
			product_list.append(product_details)
		else:
			hotel_id = hotel["product_id"]
			sql_str = "select item_name, item_code from tabItem where item_code = %s and variant_of in (select variant_of from tabItem where item_code = %s)"
			hotel_variants=tapiutils.execute_frappe_select_sql(sql_str, (hotel_id, first_hotel_id), "quotation.py", "get_variant_product_list")
			for hotel_variant in hotel_variants:
				product_name = get_product_part_name(hotel_variant.item_name)
				product_details = {"product_id":hotel_id,"product_name":product_name,"qty":hotel["qty"]}
				product_list.append(product_details)
				break

	return product_list

def get_product_name(item_code):
	product_name = ""
	sql_str = "select item_name from tabItem where item_code = %s"
	item_details=tapiutils.execute_frappe_select_sql(sql_str, (item_code), "quotation.py", "get_product_name")
	for item in item_details:
		product_name = get_product_part_name(item.item_name)
	return product_name

def get_product_part_name(item_name):
	product_name=""
	if (item_name):
		item_part_names = item_name.split("-")
		product_name = item_part_names[-1].strip()
	return product_name

def get_variant_product_list_temp(bundle_id, day, hotel_list):
	product_list = []
	for hotel in hotel_list:
		hotel_id = hotel["product_id"]
		sql_str = "select item_name from tabItem where item_code = %s"
		hotel_variants=tapiutils.execute_frappe_select_sql(sql_str, (hotel_id), "quotation.py", "get_product_list")
		for hotel_variant in hotel_variants:
			x = hotel_variant.item_name.split("-")
			product_details = {"product_id":hotel_id, "product_name":x[-1], "qty":hotel["qty"]}
			product_list.append(product_details)
			break
...
def get_rfq_supplier(rfq_id):
	...
	for supplier in suppliers:
		...
		#break
		supplier_name.append(supplier.supplier_name)
...
def create_or_update_customer_package():
	try:
		...
		if customer_package_id:
			...
			c_id=""
			cust_id=frappe.db.sql("""select name from `tabCustomer` where name=%s""",(customer_id),as_dict=1)
			if cust_id:
				c_id=cust_id[0]["name"]
			item_Details = frappe.db.sql("""select * from `tabItem` where item_code=%s""",(parent_product_bundle),as_dict=1)
			if item_Details:
